...
#undef IMMER_INVOKE_OR_EXIT
    ...
    struct TryUpdater
    {
        template <shift_t shift>
        static IMMER_FORCEINLINE update_result
        do_try_update(node_t* node,
                      ...
                      hash_t hash)
        {
            if constexpr (shift == max_shift<B>) {
                auto fst = node->collisions();
                auto lst = fst + node->collision_count();
                for (; fst != lst; ++fst)
                    if (Equal{}(*fst, k)) {
                        auto&& new_value =
                            fn(Project{}(detail::as_const(*fst)));
                ...
                return {
                    node_t::copy_collision_insert(
                        node, Combine{}(std::forward<K>(k), fn(Default{}()))),
                    true};
            } else {
                auto idx = (hash & (mask<B> << shift)) >> shift;
                auto bit = bitmap_t{1u} << idx;
                if (node->nodemap() & bit) {
                    auto offset = node->children_count(bit);
                    auto result = do_try_update<shift + B>(
                        node->children()[offset], k, fn, valueEquals, hash);
                    if (!result.node)
                        return result;
                    ...
                    if (Equal{}(*val, k)) {
                        auto&& new_value =
                            fn(Project{}(detail::as_const(*val)));
                        ...
                        auto child = node_t::make_merged(
                            shift + B,
                            Combine{}(std::forward<K>(k), fn(Default{}())),
                            hash,
                            *val,
                            Hash{}(*val));
                        IMMER_TRY {
        ...
        template <shift_t shift>
        static IMMER_FORCEINLINE update_mut_result
                          ...
                          hash_t hash)
        {
            if constexpr (shift == max_shift<B>) {
                ...
                auto v      = Combine{}(std::forward<K>(k), fn(Default{}()));
                auto mutate = node->can_mutate(e);
                auto r =
                ...
                if (node->nodemap() & bit) {
                    ...
                    auto child  = node->children()[offset];
                    if (node->can_mutate(e)) {
                        auto result = do_try_update_mut<shift + B>(
                            e, child, k, fn, valueEquals, hash);
                        if (!result.node)
                            return result;
                    ...
                    if (Equal{}(*val, k)) {
                        if (node->can_mutate(e)) {
                            auto vals = node->ensure_mutable_values(e);
                            auto&& new_value =
                                fn(Project{}(detail::as_const(vals[offset])));
                            if (valueEquals(
    ...
    champ try_update(const K& k, Fn&& fn, ValueEquals valueEquals = {}) const
    {
        ...
        auto res  = TryUpdater<
            ...
            std::decay_t<K>,
            std::decay_t<Fn>,
            ValueEquals>::template do_try_update<0>(root,
                                                    k,
                                                    detail::ref_or_fwd<Fn>(fn),
    ...
    void try_update_mut(edit_t e, K&& k, Fn&& fn, ValueEquals valueEquals)
    {
        ...
        auto res  = TryUpdater<
            ...
            std::decay_t<K>,
            std::decay_t<Fn>,
            ValueEquals>::template do_try_update_mut<0>(e,
                                                        root,
                                                        std::forward<K>(k),
                                                        detail::ref_or_fwd<Fn>(
                                                            fn),
                                                        valueEquals,
                                                        hash);
        if (!res.node)
